    ...
    public function handle($value): void
    {
        if(!is_null($value) && count($value)>0) {
          /*  $this->query->where(function ($query) use ($value){
            ...
            $this->query->where(function ($query) use ($value){
                $query->hasByNonDependentSubquery('objectRegions', function ($q) use ($value) {
                    $q->whereIn('code', $value);
                })->orDoesntHaveByNonDependentSubquery('objectRegions', function () use ($value) {
                    $this->query->hasByNonDependentSubquery('regions', function ($que) use ($value) {
                        $que->whereIn('code', $value);
                    });
                });
            });
