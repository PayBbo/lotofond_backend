<template>
    <bkt-modal :id="'authModal'" ref="authModal" :modal_class="'bkt-auth-modal '+tab">
        ...
        <template #footer>
            <button class="vk-button" @click="grantType = 'phone'" v-if="grantType === 'email'">
                {{tab=='registration'? 'Регистрация':'Вход'}} через телефон
            </button>
            <button class="vk-button" @click="grantType = 'email'" v-if="grantType === 'phone'">
        ...
        methods: {
            ...
            async submit() {
                ...
                await this.$store.dispatch(this.tab, data).then(resp => {
                    ...
                    if(error.response.data.title === 'ERR_VALIDATION_FAILED_PHONE')
                    {
